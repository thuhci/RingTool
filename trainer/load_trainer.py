        ...
        for epoch in progress_bar:
            ...
            for idx, (inputs, labels) in enumerate(train_loader):
                ...
                with autocast():
                    ...
                    # Apply physiological mask on labels/preds before computing loss
                    try:
                        filter_task = task
                        for _suffix in ("_stationary", "_motion", "_spo2"):
                            if filter_task.endswith(_suffix):
                        ...
                        # Fallback to no mask if something goes wrong
            ...
            with torch.no_grad():
                for inputs, labels in valid_loader:
                    inputs, labels = inputs.to(self.device), labels.to(self.device)
                    outputs, _ = self.model(inputs)
                    # Apply physiological mask before computing validation loss and collecting metrics arrays
                    try:
                        filter_task = task
                        for _suffix in ("_stationary", "_motion", "_spo2"):
                            if filter_task.endswith(_suffix):
